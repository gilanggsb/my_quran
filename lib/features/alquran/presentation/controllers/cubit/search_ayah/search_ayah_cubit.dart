...
part 'search_ayah_cubit.freezed.dart';
part 'search_ayah_state.dart';

class SearchAyahCubit extends Cubit<SearchAyahState> {
  ...
  }) : super(const SearchAyahState.initial());

  void init({QuranDetailParams? params, List<Ayah>? ayahs}) async {
    try {
      ayahs = ayahs;
      paramsData = params;
      emit(const SearchAyahState.loading());

      await Future.wait([_getJuzs(), _getSurahs()]);

      emit(const SearchAyahState.loaded());
      ...
      emit(SearchAyahState.failed(e));
    } catch (e) {
      emit(SearchAyahState.failed(e.toString()));
    }
  }
  ...
  void filterSurahOrJuz(String text) {
    emit(const SearchAyahState.searchingSurahOrJuz());
    if (isSurahsType) {
